...
class Decoder(nn.Module):
    def __init__(self, vocab_size):
        ...
        self.v = nn.Parameter(torch.LongTensor([self.hidden_size]))
        self.output_projection = nn.Linear(self.hidden_size, vocab_size)

    def forward(self, word, hidden, cell, encoder_hidden, max_source_len):
        ...
        for step in range(max_source_len):
            concated = torch.cat((encoder_hidden[:,step,:],h_t),1)  # shape (batch, hidden_size * 3)
            concated = self.attn_linear(concated)   # shape (batch, hidden_size)
        ...
        context = F.softmax(attention_matrix).unsqueeze(1).bmm(encoder_hidden).squeeze(1)
